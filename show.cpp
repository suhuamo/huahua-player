...
//    不做下面两个配置的添加，会导致：当窗口从后台恢复时，Qt 会清除 widget 的背景，造成黑屏。
    ...
    m_last_frame_width = 0;
    m_last_frame_height = 0;
    
    // 初始化提示标签（使用顶层窗口实现透明效果）
    m_toast_label = new QLabel(this);
    m_toast_label->setVisible(false);
    
    // 初始化定时器
    m_toast_timer = new QTimer(this);
    m_toast_timer->setSingleShot(true);
    
    // 初始化快捷键提示标签（居中显示）
    m_shortcut_hint_label = new QLabel(this);
    m_shortcut_hint_label->setVisible(false);
    
    // 初始化快捷键提示定时器
    m_shortcut_hint_timer = new QTimer(this);
    m_shortcut_hint_timer->setSingleShot(true);
}
...
//    没有视频的时候测试图片是否能正常显示
    ...
    m_toast_label->setWindowFlags(Qt::ToolTip | Qt::FramelessWindowHint);
    m_toast_label->setAttribute(Qt::WA_TranslucentBackground, true);

    
    // 设置字体
    QFont font = m_toast_label->font();
    font.setPointSize(14);
    m_toast_label->setFont(font);
    
    // 设置快捷键提示标签样式（居中显示）
    m_shortcut_hint_label->setStyleSheet(
        "QLabel {"
        "    color: #1cdb20;"
        "    border: none;"
        "}"
    ...
    m_shortcut_hint_label->setWindowFlags(Qt::ToolTip | Qt::FramelessWindowHint);
    m_shortcut_hint_label->setAttribute(Qt::WA_TranslucentBackground, true);
    
    // 设置字体
    m_shortcut_hint_label->setFont(font);
...
//    todo:这是何意为？为什么自己链接自己，为什么外面连接 &Show::sigPlay 的时候不直接连接 &Show::onPlay 得了
    ret = connect(this, &Show::sigPlay, this, &Show::onPlay);
    connect(m_toast_timer, &QTimer::timeout, this, &Show::onToastTimeout);
    connect(m_shortcut_hint_timer, &QTimer::timeout, this, &Show::onShortcutHintTimeout);
...
void Show::onFrameDimensionsChanged(int nFrameWidth, int nFrameHeight)
{
    ...
    if (m_last_frame_width == nFrameWidth && m_last_frame_height == nFrameHeight) {
        return;
    }

    m_last_frame_width = nFrameWidth;
    m_last_frame_height = nFrameHeight;
...
void Show::changeShow()
{
    ...
    qDebug() << "Show::changeShow - size:" << width() << "x" << height() << "frame:" << m_last_frame_width << "x" << m_last_frame_height;
    if(m_last_frame_width == 0 && m_last_frame_height == 0) {
        ...
        aspect_ratio = (float)m_last_frame_width / (float)m_last_frame_height;
...
{
    if (m_toast_label == nullptr) {
        return;
    }

    // 设置文本
    m_toast_label->setText(text);

    // 根据文本长度调整大小，添加一些边距
    QFontMetrics fm(m_toast_label->font());
    m_toast_label->setFixedSize(fm.width(text) + 30, fm.height() + 16);  // 左右各15px，上下各8px的边距

    // ⭐ 关键：对于顶层窗口，需要计算全局位置并调用 show()
    // 将局部坐标转换为全局坐标（左上角位置）
    QPoint globalPos = this->mapToGlobal(QPoint(10, 10));
    m_toast_label->move(globalPos);
    ...
    // 启动定时器，2秒后隐藏
    m_toast_timer->start(2000);
}

void Show::onToastTimeout()
{
    if (m_toast_label) {
        m_toast_label->hide();  // 顶层窗口使用 hide()
    }
}

void Show::showShortcutHint(const QString &text)
{
    if (m_shortcut_hint_label == nullptr) {
        return;
    }

    // 设置文本
    m_shortcut_hint_label->setText(text);

    // 根据文本长度调整大小，添加一些边距
    QFontMetrics fm(m_shortcut_hint_label->font());
    m_shortcut_hint_label->setFixedSize(fm.width(text) + 30, fm.height() + 16);  // 左右各15px，上下各8px的边距

    // 将文本显示在窗口正上方居中位置（距离顶部30px）
    int x = (this->width() - m_shortcut_hint_label->width()) / 2;
    int y = 30; // 距离顶部30px
    QPoint globalPos = this->mapToGlobal(QPoint(x, y));
    m_shortcut_hint_label->move(globalPos);
    
    // 显示标签并确保在最上层，但不获取焦点（避免影响键盘事件）
    m_shortcut_hint_label->show();
    m_shortcut_hint_label->raise();
    // 注意：不调用 activateWindow()，避免焦点转移

    // 启动定时器，2秒后隐藏
    m_shortcut_hint_timer->start(2000);
}

void Show::hideShortcutHint()
{
    if (m_shortcut_hint_label) {
        m_shortcut_hint_label->hide();
        m_shortcut_hint_timer->stop(); // 停止定时器
    }
}
